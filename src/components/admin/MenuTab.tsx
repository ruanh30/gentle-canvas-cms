...
import { Plus, Trash2, ChevronDown, ChevronRight, GripVertical, Menu, ExternalLink, Tag, Settings2 } from 'lucide-react';
...
function MenuItemEditor({ item, onUpdate, onDelete }: {
  ...
  const addChild = () => {
    const child: ThemeMenuItem = { id: `mi-${Date.now()}`, label: 'Subitem', link: '', openNewTab: false, badge: '', badgeColor: '#ef4444', icon: '', children: [] };
  ...
  return (
    <div className="border border-border rounded-lg bg-card overflow-hidden">
      <div className="flex items-center gap-2 p-3 bg-card">
        ...
        {item.badge && (
          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded shrink-0" style={{ backgroundColor: item.badgeColor, color: '#fff' }}>{item.badge}</span>
        )}
        {item.children.length > 0 && (
          <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded shrink-0">{item.children.length} sub</span>
        )}
        ...
        <div className="border-t border-border bg-muted/20 p-4 space-y-4">
          ...
          <div className="space-y-2">
            ...
            {item.children.map((child, idx) => (
              <SubitemEditor key={child.id} item={child} onUpdate={updated => updateChild(idx, updated)} onDelete={() => removeChild(idx)} />
            ))}
...
export default function MenuTab() {
  ...
  const addItem = () => {
    const newItem: ThemeMenuItem = { id: `mi-${Date.now()}`, label: 'Novo item', link: '', openNewTab: false, badge: '', badgeColor: '#ef4444', icon: '', children: [] };
    ...
    <div className="space-y-6 pt-2 max-w-2xl">
      ...
      <div className="border border-border rounded-lg bg-card">
        <div className="px-4 py-3 border-b border-border">
          <p className="text-xs font-semibold text-foreground flex items-center gap-1.5">
            <Settings2 className="h-3.5 w-3.5" /> Preferências do Menu
          </p>
        </div>
      ...
      <div className="space-y-3">
        ...
        {mm.items.map((item, idx) => (
          <MenuItemEditor key={item.id} item={item} onUpdate={updated => updateItem(idx, updated)} onDelete={() => removeItem(idx)} />
        ))}
