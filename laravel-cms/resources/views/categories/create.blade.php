...
<div class="max-w-xl">
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        @csrf
        <div>
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-primary outline-none">{{ old('description') }}</textarea>
        </div>
        <div>
        ...
        <div>
            ...
            <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm">
        </div>
