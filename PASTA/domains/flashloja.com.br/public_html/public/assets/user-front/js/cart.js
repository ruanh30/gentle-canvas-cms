...
let detail_new_price = parseFloat($('#details_new-price').data('base_price'));
if (isNaN(detail_new_price)) {
    detail_new_price = parseFloat($('#details_new-price').text());
}
var detail_old_price = parseFloat($('#details_old-price').data('old_price'));
if (isNaN(detail_old_price)) {
    detail_old_price = parseFloat($('#details_old-price').text());
}
let variant = {};
let stErr = 0;
let stErrMsg = [];
...
function buyNowDetails2() {
    ...
    if (stErr > 0) {
        stErrMsg.forEach(msg => toastr["error"](msg));
        $(".request-loader").removeClass("show");
        return;
    }

    $.get(url + ',,,' + qty + ',,,' + final_price + ',,,' + JSON.stringify(variant), function (res) {
        if (res.message) {
...
function totalPriceDetails2(qty) {
    ...
    var base_new = parseFloat($('#details_new-price').data('base_price'));
    if (isNaN(base_new)) base_new = parseFloat(detail_new_price);
    if (isNaN(base_new)) base_new = 0;

    var base_old = parseFloat($('#details_old-price').data('old_price'));
    if (isNaN(base_old)) base_old = parseFloat(detail_old_price);
    if (isNaN(base_old)) base_old = 0;

    var total = base_new;
    var old_total = base_old;
