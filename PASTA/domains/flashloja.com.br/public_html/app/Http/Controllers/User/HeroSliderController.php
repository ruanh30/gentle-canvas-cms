...
class HeroSliderController extends Controller
{
    ...
    public function storeSliderInfo(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        $request->validate(
            [
                ...
                'banner_size' => 'nullable|in:870x590,1900x600,1900x450',
                'fill_edges' => 'nullable|boolean',
                'is_active' => 'nullable|boolean',
                'serial_number' => 'required',
                'slider_img' => ($userBs->theme == 'vegetables' || $userBs->theme == 'electronics' || $userBs->theme == 'manti' || $userBs->theme == 'skinflow' || false /* was premium */) ? 'required|mimes:jpeg,jpg,png|max:5000' : '',
    ...
    public function toggleSliderStatus(Request $request)
    {
        $slider = HeroSlider::where('user_id', Auth::guard('web')->user()->id)
            ->where('id', $request->slider_id)
            ->firstOrFail();
        $slider->is_active = $slider->is_active ? 0 : 1;
        $slider->save();
