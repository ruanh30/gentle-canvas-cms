...
class ItemCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('user.itemcategory.index', ['language' => request('language', 'pt-BR')]);
    }
    ...
    public function delete(Request $request)
    {
        ...
        if ($itemCount > 0 && !$request->has('force_delete')) {
            // Return JSON for AJAX confirmation flow
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'needs_confirm' => true,
                    'message' => "Esta categoria possui {$itemCount} produto(s) vinculado(s). Deseja excluir a categoria e todos os seus produtos?",
                    'category_id' => $category->id,
                ]);
            }
            Session::flash('warning', "Esta categoria possui {$itemCount} produto(s) vinculado(s). Deseja excluir mesmo assim?");
            return back();
        }

        // If force_delete, delete items first
        if ($itemCount > 0) {
            foreach ($category->items as $item) {
                // Delete item images
                if ($item->thumbnail) {
                    @unlink(public_path('assets/front/img/user/items/') . $item->thumbnail);
                }
                $item->delete();
            }
        }
