...
class HomeController extends Controller
{
    public function userDetailView($domain)
    {
        ...
        $data['recent_items'] = DB::table('user_items')->where('user_items.user_id', $user->id)
            ->Join('user_item_contents', 'user_items.id', '=', 'user_item_contents.item_id')
            ->join('user_item_categories', 'user_item_contents.category_id', '=', 'user_item_categories.id')
            ->select('user_items.*', 'user_items.id AS item_id', 'user_item_contents.*', 'user_item_categories.name AS category', 'user_item_categories.slug AS category_slug')
            ->where('user_item_contents.language_id', '=', $userCurrentLang->id)
            ...
            ->where([['user_items.status', 1], ['user_item_categories.status', '=', 1]])
            ->orderBy('user_items.id', 'DESC')
            ->take($shopSet->latest_item_count ?? 8)
            ->get();
        ...
        $data['seo'] = SEO::where('language_id', $uLang)->where('user_id', $user->id)->select('home_meta_description', 'home_meta_keywords')->first();
        ...
        } elseif ($data['ubs']->theme == 'premium') {
            return view('user-front.premium.index', $data);
        } else {
    ...
    public function contactView()
    {
        ...
        $ubs = app('userBs');
        if (!empty($ubs) && false /* legacy check removed */) {
            abort(404); // legacy
        }
    ...
    public function userBlogs(Request $request, $domain)
    {
        ...
        $data['seo'] = SEO::where('language_id', $userCurrentLang->id)->where('user_id', $user->id)->select('blogs_meta_keywords', 'blogs_meta_description')->first();
    ...
    public function userAbout($domain)
    {
        ...
        $data['seo'] = SEO::where('language_id', $userCurrentLang->id)->where('user_id', $user->id)->select('about_page_meta_keywords', 'about_page_meta_description')->first();
