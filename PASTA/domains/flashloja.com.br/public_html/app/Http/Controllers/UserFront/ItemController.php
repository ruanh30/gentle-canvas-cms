...
class ItemController extends Controller
{
    public function cart($domain)
    {
        ...
        $data['cart'] = Session::get($cartKey, []);

        // When cart is empty, show the cart page (do not redirect to home)
        if (empty($data['cart']) || !is_array($data['cart'])) {
            $data['cart'] = [];
        }

        $data['totalQty'] = count($data['cart']) ? array_sum(array_column($data['cart'], 'qty')) : 0;
        $data['totalCart'] = count($data['cart']) ? array_sum(array_column($data['cart'], 'total')) : 0;
