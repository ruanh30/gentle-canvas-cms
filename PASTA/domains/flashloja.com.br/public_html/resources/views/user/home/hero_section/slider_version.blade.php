                  ...
                  <table class="table table-striped mt-3" id="basic-datatables">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ __('Ativar/Desativar') }}</th>
                    ...
                    <tbody>
                      @foreach ($sliders as $slider)
                        <tr>
                          ...
                          <td>{{ $slider->title }}</td>
                          <td>
                            <span class="badge badge-{{ $slider->is_active == 1 ? 'success' : 'danger' }}">{{ $slider->is_active == 1 ? __('Ativo') : __('Inativo') }}</span>
                            <form class="d-inline-block ml-2" action="{{ route('user.home_page.hero.toggle_slider') }}" method="post">
                              @csrf
                              <input type="hidden" name="slider_id" value="{{ $slider->id }}">
                              <button type="submit" class="btn btn-sm btn-{{ $slider->is_active == 1 ? 'warning' : 'success' }}">
                                {{ $slider->is_active == 1 ? __('Desativar') : __('Ativar') }}
                              </button>
                            </form>
                          </td>
                          <td>
