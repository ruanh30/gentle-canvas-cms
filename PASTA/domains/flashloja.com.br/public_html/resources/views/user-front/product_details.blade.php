              ...
              <div class="product-single-gallery">
                <div class="slider-thumbnails2">
                  ...
                  @foreach ($product->item->sliders as $slide)
                    @php
                      $sliderPath = 'assets/front/img/user/items/slider-images/' . $slide->image;
                      $sliderFullPath = public_path($sliderPath);
                      $fallbackPath = 'assets/front/img/user/items/thumbnail/' . $product->item->thumbnail;
                      $slideSrc = file_exists($sliderFullPath) ? asset($sliderPath) : asset($fallbackPath);
                    @endphp
            ...
            <div class="product-single-details">
              ...
              <div class="product-action">
                ...
                @if ($shop_settings->catalog_mode != 1)
                  ...
                  <button class="btn btn-sm btn-danger radius-md" type="button" aria-label="Comprar agora"
                    data-bs-toggle="tooltip" data-placement="top"
                    title="{{ $keywords['Buy_Now'] ?? __('Buy Now') }}" onclick="buyNowDetails2()">
                    <i class="fas fa-bolt"></i><span>{{ $keywords['Buy_Now'] ?? __('Buy Now') }}</span>
                  </button>

                  <button class="btn btn-sm btn-primary radius-md ms-2" type="button" aria-label="Add to cart"
                    data-bs-toggle="tooltip" data-placement="top"
...
@section('scripts')
<script>
if (typeof window.buyNowDetails2 !== 'function') {
  window.buyNowDetails2 = function () {
    $(".request-loader").addClass("show");
    let $input = $(".item_quantity_details input");
    let qty = parseInt($input.val());
    let item_id = $('#details_item_id').val();
    let url = mainurl + "/add-to-cart/" + item_id;
    let final_price = (typeof totalPriceDetails2 === 'function') ? totalPriceDetails2(qty) : 0;

    if (typeof stErr !== 'undefined' && stErr > 0) {
      if (Array.isArray(stErrMsg)) stErrMsg.forEach(msg => toastr["error"](msg));
      $(".request-loader").removeClass("show");
      return;
    }

    $.get(url + ',,,' + qty + ',,,' + final_price + ',,,' + JSON.stringify(typeof variant !== 'undefined' ? variant : {}), function (res) {
      if (res.message) {
        window.location.href = mainurl + '/cart';
      } else {
        toastr["error"](res.error || 'Erro ao adicionar ao carrinho');
        $(".request-loader").removeClass("show");
      }
    }).fail(function () {
