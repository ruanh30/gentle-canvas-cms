...
@section('content')
  ...
  @if ($isPremium && (empty($cart) || count($cart) === 0))
    <style>
      .pm-cart-wrap{ padding: 40px 16px; background: {{ $bg }}; }
      .pm-cart-card{ max-width: 520px; margin: 0 auto; background: {{ $bg }}; border: 1px solid {{ $border }}; border-radius: 12px; padding: 28px; box-shadow: 0 4px 24px rgba(0,0,0,.04); text-align: center; }
      .pm-cart-title{ font-size: 24px; font-weight: 700; margin: 0 0 8px; color: {{ $fg }}; }
      .pm-cart-desc{ font-size: 14px; margin: 0 0 18px; color: {{ $mutedFg }}; }
      .pm-cart-btn{ display:inline-block; padding: 12px 16px; background: {{ $primary }}; color: {{ $primaryFg }}; border-radius: 10px; text-decoration:none; font-weight:600; }
      .pm-cart-btn:hover{ opacity:.92; color: {{ $primaryFg }}; }
      .pm-cart-link{ display:block; margin-top: 14px; font-size: 13px; color: {{ $mutedFg }}; text-decoration: none; }
      .pm-cart-link:hover{ color: {{ $fg }}; }
    </style>

    <div class="pm-cart-wrap">
      <div class="pm-cart-card">
        <h1 class="pm-cart-title">{{ $pageHeading->cart_page ?? __('Carrinho') }}</h1>
        <p class="pm-cart-desc">{{ __('Seu carrinho está vazio.') }}</p>
        <a class="pm-cart-btn" href="{{ route('front.user.shop', getParam()) }}">{{ __('Ver produtos') }}</a>
        <a class="pm-cart-link" href="{{ route('front.user.detail.view', getParam()) }}">{{ __('Voltar para a loja') }}</a>
      </div>
    </div>
  @else
  <!-- Cart Start -->
