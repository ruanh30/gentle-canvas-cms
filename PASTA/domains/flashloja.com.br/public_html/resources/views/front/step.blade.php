        ...
        <form id="#authForm" action="{{ route('front.checkout.view') }}" method="post" enctype="multipart/form-data">
          ...
          <div class="title">
            <h3>{{ __('Signup') }}</h3>
              ...
              $defaultCategory = optional($categories->first())->unique_id;
            @endphp
            <input type="hidden" name="category" value="{{ $defaultCategory }}">
          <div class="form-group mb-20">
            <input type="text" class="form-control" name="username" placeholder="Nome da sua Loja"
              value="{{ old('username') }}" required>
            @if ($hasSubdomain)
              <p class="mb-0 small">
                {{ __('Assim ficara o link de sua loja') }}:
